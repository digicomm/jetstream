...
onMounted(async () => {
  ...
  dTable = table.DataTable({
    ...
    ajax: function (data, callback, settings) {
      ...
      axios({
        method: tableMethod.value,
        url: tableMethod.value === 'get' ? route('inventory.variancereport.show', {variancereport: 'data'}) : route('inventory.variancereport.store')
      })
          .then(response => {
      ...
      buttons: [
        {
          ...
          action: () => {
            downloadFile(route('inventory.variancereport.create'), getFileName())
          }
        },
        {
          extend: 'print',

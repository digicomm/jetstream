...
onMounted(async () => {
  ...
  dTable = table.DataTable({
    ajax: function (data, callback) {
      ...
      axios.patch(route('inventory.view.update', {view: 'empty'}))
          .then(response => {
    ...
    columns: [
      ...
      {data: 'lvl_one_sort', className: 'text-nowrap'},
      {data: 'lvl_two_sort', className: 'text-nowrap'},
      {data: 'lvl_three_sort', className: 'text-nowrap'},
      {data: 'sort_key', className: 'text-nowrap'}
    ],
    buttons: {
      dom: {
        buttonLiner: {
      ...
      buttons: [
        {
          ...
          action: () => {
            downloadFile(route('inventory.view.create', {view: 'empty'}), getFileName())
                .catch(response => {
  ...
  <AppLayout :title="title" layout="">
    ...
    <div class="mx-auto p-4 lg:p-4 w-full">
      ...
      <DCard container-class="relative" show-overflow>
        ...
        <DTableLoading v-if="tableLoading" loading-message="Refreshing Empty Bins"/>
      </DCard>
      <!--
